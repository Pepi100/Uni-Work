...
class GrupIngrediente extends React.Component {
    ...
    constructor(props) {
        ...
        this.clickResetare = this.clickResetare.bind(this);
        //construiesc starea de cantitati selectate
        let grIngrediente = this.props.json.grup_ingrediente
        for (let i = 0; i < grIngrediente.length; i++) {
            this.vectIngrediente[grIngrediente[i].tip] = {}
    ...
    render() {
        ...
        if (this.totalCalorii > 2000) {
            rez.push(React.createElement("p", null, mesaje["peste limita"]))
        } else {
            if (this.totalCalorii >= 1000)
                rez.push(React.createElement("p", null, mesaje[2000]))
            else {
                if (this.totalCalorii >= 500)
                    rez.push(React.createElement("p", null, mesaje[1000]))
                else {
                    if (this.totalCalorii >= 200)
                        rez.push(React.createElement("p", null, mesaje[500]))
                    else {
                        rez.push(React.createElement("p", null, mesaje[200]))
                    }
                }
            }

        }
        rez.push(React.createElement("button", { onClick: this.clickResetare }, `Resetare`))
...
class ComponentaSimpla extends React.Component {
    ...
    render() {
        ...
        return [React.createElement("p", { title: "text tooltip", onClick: this.exempluClick, className: "test" },
                `whatever  ${this.props.proprietate} ${new Date()}`),
            React.createElement(ComponentaSimpla2, { title: "text tooltip", exempluClick: this.exempluClick, className: "test", ceva: 1 })
        ];
    }
}
