...
void rezolvare(BikeTrail trail){
    ...
    cout<<v[a]<<v[b]<<v[c];
    cout<<"Suma vizitatorilor: "<<v[a].getVizitatori()+v[b].getVizitatori()+v[c].getVizitatori()<<'\n';

}


void rezolvare1(BikeTrail trail){
    int a, b, c;
    vector<InterestPoint> v = trail.getPuncte();
    int vMaxim = -1;
    int dmin;
    for(int i = 0; i<trail.getPuncte().size();i++){
        for(int j = 0; j<trail.getPuncte().size();j++){
            for(int k = 0; k<trail.getPuncte().size();k++){
                if( i!=j && j!=k && i!=k && v[i].getVizitatori()+v[j].getVizitatori()+v[k].getVizitatori() > vMaxim){
                    vMaxim = v[i].getVizitatori()+v[j].getVizitatori()+v[k].getVizitatori();
                    dmin = v[i].getDistance(v[j])+v[j].getDistance(v[k]);
                    a=i,b=j,c=k;
                    if(v[i].getDistance(v[k])+v[k].getDistance(v[j]) < dmin ){
                        a=i,b=k,c=j;
                        dmin = v[i].getDistance(v[k])+v[k].getDistance(v[j]);
                    }
                    if(v[j].getDistance(v[i])+v[i].getDistance(v[k]) < dmin ){
                        a=j,b=i,c=k;
                        dmin = v[i].getDistance(v[k])+v[k].getDistance(v[j]);
                    }
                }
    ...
    cout<<"Numarul maxim de vizitatori dintre 3 puncte: "<<vMaxim<<"\n Punctele:\n";
    cout<<v[a]<<v[b]<<v[c];
