...
isNothing n = maybe true (const false) n
...
isJust n = maybe false (const true) n
...
--maybeBind f m = maybe nothing (maybe nothing f) m
...
instance MaybeClass CMaybe where
  ...
  maybe :: b -> (a -> b) -> CMaybe a -> b
  maybe b j a = getCMaybe a b j
