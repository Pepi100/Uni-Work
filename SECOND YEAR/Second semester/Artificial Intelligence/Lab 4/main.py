...
class Graph:  # graful problemei
   ...
   def __init__(self, start, scopuri):
    ...
    # ERATI LA TRAINING SI AVEATI PIZZA SI NU ATI DAT SI VOI NIMIC LA BOSCHETARUL DE AFARA
   # va genera succesorii sub forma de noduri in arborele de parcurgere
   def succesori(self, nodCurent):
        listaSuccesori = []
        for istiva in range(len(nodCurent.info)):
            if len(nodCurent.info[istiva]) == 0:
                continue

            copieStiva = copy.deepcopy(nodCurent.info)
            bloc = copieStiva[istiva].pop()
            for istiva2 in range(len(nodCurent.info)):
                if istiva == istiva2:
                    continue
                ...
                nodNou = NodParcurgere(stareNoua, nodCurent.g+1, h=self.estimeaza_h(stareNoua), parinte= nodCurent)
   ...
   def estimeaza_h(self, infoNod):
       # return self.lista_h[infoNod]
        return 0
   def valideaza(self):
       nrStive = len(self.start)
       cond1 = all([len(scop) == nrStive for scop in self.scopuri])

       blocuriInitiale = sorted(sum(self.start, start=[]))
       cond2 = all([ blocuriInitiale == sorted(sum(scop, start=[])) for scop in self.scopuri])
...
def calculeazaStive(sirStiva):
    return [st.strip().split() if st.strip() != '#' else [] for st in sirStiva.strip().split("\n")]
